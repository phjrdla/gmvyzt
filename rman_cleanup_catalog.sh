...
#oktesttest_connect=`gpg -qd $SECFILE | grep -i $ORACLE_SID_BASE | grep -i TEST_CONNECT | cut -d ":" -f3`
...
#oktestsys=`gpg -qd $SECFILE | grep -i $ORACLE_SID_BASE | grep -w SYS | cut -d ":" -f3`
...
#oktestadmdzdba=`gpg -qd $SECFILE | grep -i $ADMIN_INSTANCE | grep -w DZDBA | cut -d ":" -f3`
...
#oktestrman=`gpg -qd $SECFILE | grep -w $RMAN_SID | grep -w ${rmanowner} | cut -d ":" -f3`
