...
function mail_error {
  ...
  title="ERROR within a :  '${ORACLE_BASE}/admin/backup/scripts/${pgm}.sh  -d ${SID}  -r RMAN_SID:DD2500'  execution."
...
pgm="rman_backup_arc_ddup_light"
...
USAGE="Usage on SECONDARY Db server : /u01/app/oracle/admin/backup/scripts/${pgm}.sh  -d SID -r RMAN_SID:DD2500"
...
# -----------------------------------------
            ...
            then
              ...
              if [ "${RMAN_SID}" != "UNKNOWN" ] ;
              then
                ${ORACLE_HOME}/bin/rman target sys/${oktestsys}@${ORACLE_RAC_SID} catalog ${rmanowner}/${oktestrman}@${RMAN_SID} cmdfile ${cmd}  >>${cmdlog}
              else
